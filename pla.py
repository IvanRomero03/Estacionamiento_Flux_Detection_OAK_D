...
def create_pipeline():
    ...
    det_nn.setBlobPath(blobconverter.from_zoo(
        name="vehicle-license-plate-detection-barrier-0106", shaves=shaves))
    ...
    veh_nn.setBlobPath(blobconverter.from_zoo(
        name="vehicle-detection-adas-0002", shaves=shaves))
    ...
    rec_nn.setBlobPath(blobconverter.from_zoo(
        name="license-plate-recognition-barrier-0007", shaves=shaves))
    ...
    attr_nn.setBlobPath(blobconverter.from_zoo(
        name="vehicle-attributes-recognition-barrier-0039", shaves=shaves))
...
def lic_thread(det_queue, rec_queue):
    ...
    while running:
        try:
            ...
            license_detections = [
                detection for detection in dets if detection.label == 2]

            for detection in license_detections:
                bbox = frame_norm(
                    orig_frame, (detection.xmin, detection.ymin, detection.xmax, detection.ymax))
...
def rec_thread(q_rec, q_pass):
    ...
    while running:
        ...
        rec_results = [(cv2.resize(rec_frame, (200, 64)),
                        decoded_text)] + rec_results[:9]
        fps.tick('rec')
...
def attr_thread(q_attr, q_pass):
    ...
    while running:
        ...
        attr_results = [(attr_frame, color, type, color_prob,
                         type_prob)] + attr_results[:9]
        ...
        while should_run():
            ...
            if debug:
                ...
                for detection in license_detections + vehicle_detections:
                    bbox = frame_norm(
                        debug_frame, (detection.xmin, detection.ymin, detection.xmax, detection.ymax))
                    cv2.rectangle(
                ...
                for rec_img, rec_text in rec_results:
                    ...
                    cv2.putText(rec_placeholder_img, rec_text, (5, 25),
                ...
                for attr_img, attr_color, attr_type, color_prob, type_prob in attr_results:
                    ...
                    cv2.putText(attr_placeholder_img, attr_color, (15, 30),
                                cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0))
                    cv2.putText(attr_placeholder_img, attr_type, (15, 50),
                                cv2.FONT_HERSHEY_TRIPLEX, 0.5, (0, 255, 0))
                    cv2.putText(attr_placeholder_img, f"{int(color_prob * 100)}%",
                                (150, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0))
                    cv2.putText(attr_placeholder_img, f"{int(type_prob * 100)}%",
                                (150, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0))
